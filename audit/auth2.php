...
if ($connection) {
	...
	if ($row_check['num'] > 0) {
		...
		username = '$username' AND active_status = '1'AND audit_status = '1'";
		...
		if ($check == 1) {
			...
			echo $mypassword = stringInsert($password, $secure_text, $secure_pointer);

			if ($mypassword == $row['password']) {   // Login สำเร็จ //
				if ($row['user_level'] == 3 && $row['user_level'] == 4) {
					$_SESSION['user_id'] = $row['user_id'];
