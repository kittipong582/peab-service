...
WHERE customer_branch_id = '$customer_branch_id' $condition $search_condition $time AND job_id NOT in(select job_id from  tbl_group_pm_detail)";
...
while ($row = mysqli_fetch_assoc($rs)) {
    ...
    $pm_list = "";
    if ($row['job_type'] == 2 && $row['list_pm_job'] != "") {

        $pm_list =  $row['list_pm_job'] . " / " . date('Y', strtotime($row['create_datetime']));
    }
...
LEFT JOIN tbl_branch e ON a.care_branch_id = e.branch_id 
WHERE  a.customer_branch_id = '$customer_branch_id' $condition1)   $search_condition1 $time";
...
while ($row_chk = mysqli_fetch_assoc($rs_chk)) {
    ...
    while ($row_detail = mysqli_fetch_assoc($rs_detail)) {
        ...
        $pm_list = "";
        if ($row_detail['list_pm_job'] != "") {
            $pm_list =  $row_detail['list_pm_job'] . " / " . date('Y', strtotime($row_detail['create_datetime']));
        }
    }
    ...
    if ($row_chk['cancel_datetime'] != null) {
        ...
        $status = "ปิดงาน" . "<br/>" . "<font color='red'>" . strip_tags(str_replace(" ", "", $row_chk['close_note'])) . "</font>";
    } else if ($row_chk['finish_service_time'] != null) {
        $status = "รอปิดงาน";
    } else if ($row_chk['start_service_time'] != null) {
        $h = date('H', strtotime('NOW')) - date('H', strtotime($row_chk['start_service_time']));
    ...
    <tbody>
        ...
        ?>
            <tr>
                ...
                <td class="text-center"><?php echo "<a href='view_cm.php?id=" . $row['job_id'] . "&&type=" . $row['type'] . "'  target='_blank'>" . $row['job_no'] . "</a>" ?><?php echo "</br>" . date("d-m-Y", strtotime($row['job_create'])) ?><?php
                                                                                                                                                                                                                                                    echo "</br>" . $row['pm_list'];
                                                                                                                                                                                                                                                    ?></td>
                <td class="text-center"><?php echo date("d-m-Y H:i", strtotime($row['appointment_date'])) ?><br><?php echo $row['time'] ?></td>

                <td class="text-left"><?php echo $row['contact']; ?></td>
                <td class="text-left"><?php echo $row['team']; ?></td>
                <td class="text-left"><?php echo $row['status'] ?>
                    <br><button type="button" class="btn btn-danger btn-sm" onclick="DeleteJob('<?php echo $row['job_id'] ?>','<?php echo $row['type'] ?>')">ลบ</button>
                </td>
