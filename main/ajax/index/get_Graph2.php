...
for ($i = 01; $i <= 12; $i++) {
    ...
    WHERE MONTH(group_date) = '$i' and YEAR(group_date) = '$sday'";
    ...
    $sql = "SELECT COUNT(group_pm_id) as total_sum,group_date FROM tbl_group_pm a
        LEFT JOIN tbl_user g ON a.create_user_id = g.user_id
    WHERE MONTH(group_date) = '$i' and YEAR(group_date) = '$sday' and start_service_time is not null and finish_service_time is not null ";
