...
$array_data = array();
$year = mysqli_real_escape_string($connect_db, $_POST['year']);
$month = mysqli_real_escape_string($connect_db, $_POST['month']);

$last_year = date('Y', strtotime($year . " - 1 year"));

$type = mysqli_real_escape_string($connect_db, $_POST['type']);

$sql_job = "SELECT COUNT(*) AS total_job FROM tbl_job WHERE YEAR(appointment_date) = '$year' AND MONTH(appointment_date) = '$month' AND job_type = '$type'";
...
$sql_job_finish = "SELECT COUNT(*) AS total_finish FROM tbl_job WHERE YEAR(appointment_date) = '$year' AND MONTH(appointment_date) = '$month'  AND job_type = '$type'
