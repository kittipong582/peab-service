...
echo$sql = "SELECT * FROM tbl_qc_checklist WHERE topic_id = '$topic_id' $condition ORDER BY list_order ASC";
...
<div class="row">
    <div class="col-md-12 col-sm-12 mt-1">
        <button type="button" class="btn btn-info btn-sm m-t-n-xs mr-1" id="ShowButton" onclick="ShowButtonSet();"><i class="fa fa-cogs"></i> ตั้งค่าจัดลำดับ</button>
        <button type="button" class="btn btn-info btn-sm m-t-n-xs mr-1" id="HideButton" onclick="HideButtonSet();" style="display: none;"><i class="fa fa-reply"></i> ยกเลิกจัดลำดับ</button>
        <button type="button" class="btn btn-warning btn-sm m-t-n-xs mr-1" id="SortButton" onclick="SortData();" style="display: none;"><i class="fa fa-retweet"></i> จัดลำดับ</button>
    </div>
    ...
    <table class="table table-bordered border-top table-hover" id="myTable" style="width:100%">
        <thead>
            <tr>
                ...
                <th class="text-center" style="width:50%;">Qc</th>
            ...
            <?php while ($row = mysqli_fetch_assoc($res)) { ?>
                <tr id="tr_<?php echo $row["checklist_id"]; ?>" class="tr_move">
                    ...
                    <td class="text-center p-2">
                        <button class="btn btn-xs <?php echo ($row['active_status'] == 1) ? 'btn-primary' : 'btn-danger'; ?> w-100" onclick="ChangeStatus(this,'<?php echo $row['checklist_id']; ?>')">
                            <?php echo ($row['active_status'] == 1) ? 'ใช้งาน' : 'ไม่ใช้งาน'; ?>
                    </td>
                    <td class="text-center pl-3 pr-3 pt-2">
                        <button class="btn btn-warning btn-xs w-100 mb-2" onclick="GetModalEdit('<?php echo $row['topic_id']; ?>','<?php echo $row['checklist_id']; ?>')">แก้ไข</button>
                        <button class="btn btn-danger btn-xs w-100 mb-2" onclick="DeleteList('<?php echo $row['checklist_id'] ?>')">ลบ</button>
                    </td>
                </tr>
            <?php } ?>
        </tbody>
