...
if ($_POST['job_type'] == 'x' || $_POST['job_type'] == 2) {
    ...
    $sql_chk = "SELECT a.create_group_datetime,a.group_date,a.group_pm_id,a.start_service_time,a.finish_service_time,e.fullname,f.fullname as responsible_name,a.close_datetime,i.fullname AS close_name,g.branch_name as responsible_branch FROM tbl_group_pm a
...
LEFT JOIN tbl_job a ON a.job_id = b.job_id) $condition2";
    ...
    while ($row_chk = mysqli_fetch_array($rs_chk)) {
        ...
        $sql_group = "SELECT a.job_id,a.job_no,a.job_id,b.sub_type_name,d.customer_name,d.customer_code,c.branch_name,c.branch_code,a.create_datetime,
h.serial_no,a.appointment_date,a.appointment_time_start,a.appointment_time_end,h.brand_id,h.model_id,c.district_id,h.product_type,
a.cancel_datetime,a.close_user_id,a.finish_service_time,a.start_service_time,a.close_datetime,a.close_note
...
$objPHPExcel->getActiveSheet()->getStyle($columnCharacter[0] . $rowCell . ':' . $columnCharacter[24] . ($rowCell))->getAlignment()->setWrapText(true);
$objPHPExcel->getActiveSheet()->getStyle($columnCharacter[0] . $rowCell . ':' . $columnCharacter[24] . ($rowCell))->applyFromArray($styleHeader);
...
foreach ($work_list as $row) {
    ...
    //ประเภทเครื่อง
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[13] . $rowCell, $row['product_type']);
    $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[13] . $rowCell . ':' . $columnCharacter[13] . ($rowCell));
    ...
    //เครื่อง
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[15] . $rowCell, $row['product']);
    $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[15] . $rowCell . ':' . $columnCharacter[15] . ($rowCell));
    ...
    $objPHPExcel->getActiveSheet()->getStyle($columnCharacter[0] . $rowCell . ':' . $columnCharacter[24] . ($rowCell))->applyFromArray($styleTextCenter)->getAlignment()->setWrapText(true);
