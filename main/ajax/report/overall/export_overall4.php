...
if ($_POST['job_type'] == 'x' || $_POST['job_type'] == 2) {
     ...
     WHERE j.group_pm_id = '{$row_chk['group_pm_id']}';";
       ...
       $sql_check = "SELECT * FROM tbl_group_pm a
        LEFT JOIN tbl_group_pm_detail b ON a.group_pm_id = b.group_pm_id
         WHERE a.group_pm_id = '{$row_chk['group_pm_id']}'";
    ...
    WHERE a.job_id = '{$row_check['job_id']}' ;";
        ...
        $interval = abs($datetime2 - $datetime1);
        $hours = round($interval / 3600); // หารด้วย 3600 เพื่อแปลงเป็นชั่วโมง

        $sla_status = "";
        if ($datetime2 != NULL) {
            if ($hours > 48) { // 48 ชั่วโมงเท่ากับ 2880 นาที
                $sla_status = $hours . "";
            } else {
                $sla_status = $hours . "";
            }
        ...
        // $datetime_start = strtotime($row_chk['start_service_time']);
        // $datetime_finish = strtotime($row_chk['finish_service_time']);
        // $interval_seconds = abs($datetime_finish - $datetime_start);
        // $worktime = round($interval_seconds / 3600); // แปลงเป็นชั่วโมง

        // คำนวณระยะเวลาระหว่างเวลาเริ่มต้นและเวลาที่เสร็จสิ้นการบริการ
        $datetime_start = strtotime($row_chk['start_service_time']);
        $datetime_finish = strtotime($row_chk['finish_service_time']);
        $interval_seconds = abs($datetime_finish - $datetime_start);

        // แปลงเป็นชั่วโมงและนาที
        $worktime_hours = floor($interval_seconds / 3600); // แปลงเป็นชั่วโมง
        $worktime_minutes = round(($interval_seconds % 3600) / 60); // แปลงเป็นนาทีและคิดเป็นเศษนาทีที่เหลือ

        // เพิ่มจำนวนนาทีเข้าไปในผลลัพธ์
        $worktime = $worktime_hours . ' : ' . $worktime_minutes . ' นาที';
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[12] . $rowCell, 'ระยะเวลาทำงาน(ชั่วโมง)');
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[13] . $rowCell, 'SLA(ชั่วโมง)');
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[14] . $rowCell, 'สถานะ SLA');
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[15] . $rowCell, 'ประเภทเครื่อง');
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[16] . $rowCell, 'หมายเลขเครื่อง');
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[17] . $rowCell, 'เครื่อง');
$objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[17] . $rowCell . ':' . $columnCharacter[17] . ($rowCell));
$objPHPExcel->getActiveSheet()->getColumnDimension($columnCharacter[17])->setWidth(40);

$objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[18] . $rowCell, 'อาการแจ้งซ่อม');
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[27] . $rowCell, 'so_no');
...
foreach ($work_list as $row) {
    ...
    //SLA
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[12] . $rowCell, $row['worktime']);
    $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[12] . $rowCell . ':' . $columnCharacter[12] . ($rowCell));

    //ระยะเวลาทำงาน
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[13] . $rowCell, $row['sla_status']);
    $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[13] . $rowCell . ':' . $columnCharacter[13] . ($rowCell));
    //สถานะ SLA
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[14] . $rowCell, $row['sla_type']);
    $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[14] . $rowCell . ':' . $columnCharacter[14] . ($rowCell));

    //ประเภทเครื่อง
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[15] . $rowCell, $row['product_type']);
    $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[15] . $rowCell . ':' . $columnCharacter[15] . ($rowCell));

    //หมายเลขเครื่อง
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[16] . $rowCell, $row['serial_no']);
    $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[16] . $rowCell . ':' . $columnCharacter[16] . ($rowCell));

    //เครื่อง
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[17] . $rowCell, $row['product']);
    $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[17] . $rowCell . ':' . $columnCharacter[17] . ($rowCell));

    //อาการเสีย
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[18] . $rowCell, $symptom);
    $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[18] . $rowCell . ':' . $columnCharacter[18] . ($rowCell));

    //วิธีแก้
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[19] . $rowCell, $reason);
    $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[19] . $rowCell . ':' . $columnCharacter[19] . ($rowCell));



    //ทีมงาน
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[20] . $rowCell, $row['responsible_branch']);
    $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[20] . $rowCell . ':' . $columnCharacter[20] . ($rowCell));

    //ชาสงดูแล
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[21] . $rowCell, $row['responsible_name']);
    $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[21] . $rowCell . ':' . $columnCharacter[21] . ($rowCell));

    //สถานะ
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[22] . $rowCell, $row['job_status']);
    $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[22] . $rowCell . ':' . $columnCharacter[22] . ($rowCell));


    //สถานะ
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[23] . $rowCell, number_format($row['total_income'], 2));
    $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[23] . $rowCell . ':' . $columnCharacter[23] . ($rowCell));

    //สถานะ
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[24] . $rowCell, number_format($row['cash'], 2));
    $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[24] . $rowCell . ':' . $columnCharacter[24] . ($rowCell));
    //สถานะ
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[25] . $rowCell, number_format($row['transfer'], 2));
    $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[25] . $rowCell . ':' . $columnCharacter[25] . ($rowCell));

    //สถานะ
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[26] . $rowCell, number_format($row['bill'], 2));
    $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[26] . $rowCell . ':' . $columnCharacter[26] . ($rowCell));
    ...
    if ($row['type'] == 1) {
        $sql_staff = "SELECT *FROM tbl_job_staff a WHERE a.job_id = '{$row['job_id']}'";
        $result_staff = mysqli_query($connection, $sql_staff);
        $num_staff = mysqli_num_rows($result_staff);

    } elseif ($row['type'] == 2) {
        $sql_staff = "SELECT a.*,c.fullname,d.branch_name FROM tbl_job_staff a LEFT JOIN tbl_group_pm_detail b ON a.job_id = b.group_pm_id LEFT JOIN tbl_user c ON a.staff_id = c.user_id LEFT JOIN tbl_branch d ON c.branch_id = d.branch_id
        WHERE a.job_id = '{$row['job_id']}'";
        $result_staff = mysqli_query($connection, $sql_staff);
        $num_staff = mysqli_num_rows($result_staff);
    ...
    while ($row_staff = mysqli_fetch_array($result_staff)) {
        ...
        if ($num_staff > 0) {
            ...
            //order_no
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[0] . $rowCell, );
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[0] . $rowCell . ':' . $columnCharacter[0] . ($rowCell));

            //ประเภทงาน
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[1] . $rowCell, );
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[1] . $rowCell . ':' . $columnCharacter[1] . ($rowCell));

            //ลูกค้า
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[2] . $rowCell, );
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[2] . $rowCell . ':' . $columnCharacter[2] . ($rowCell));

            //ลูกค้า
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[3] . $rowCell, );
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[3] . $rowCell . ':' . $columnCharacter[3] . ($rowCell));

            //ลูกค้า
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[4] . $rowCell, );
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[4] . $rowCell . ':' . $columnCharacter[4] . ($rowCell));

            //ชื่อร้าน
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[5] . $rowCell, );
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[5] . $rowCell . ':' . $columnCharacter[5] . ($rowCell));

            //ที่อยู่ร้าน
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[6] . $rowCell, );
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[6] . $rowCell . ':' . $columnCharacter[6] . ($rowCell));

            //////////////ประกัน
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[7] . $rowCell, );
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[7] . $rowCell . ':' . $columnCharacter[7] . ($rowCell));

            //  วันที่เปิดงาน
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[8] . $rowCell, );
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[8] . $rowCell . ':' . $columnCharacter[8] . ($rowCell));

            //วันที่นัดหมาย
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[9] . $rowCell, );
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[9] . $rowCell . ':' . $columnCharacter[9] . ($rowCell));



            //check_in
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[10] . $rowCell, );
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[10] . $rowCell . ':' . $columnCharacter[10] . ($rowCell));


            //check_out
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[11] . $rowCell, );
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[11] . $rowCell . ':' . $columnCharacter[11] . ($rowCell));


            //SLA
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[12] . $rowCell, );
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[12] . $rowCell . ':' . $columnCharacter[12] . ($rowCell));

            //ระยะเวลาทำงาน
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[13] . $rowCell, );
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[13] . $rowCell . ':' . $columnCharacter[13] . ($rowCell));
            //สถานะ SLA
            ...
            //ประเภทเครื่อง
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[15] . $rowCell, );
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[15] . $rowCell . ':' . $columnCharacter[15] . ($rowCell));

            //หมายเลขเครื่อง
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[16] . $rowCell, );
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[16] . $rowCell . ':' . $columnCharacter[16] . ($rowCell));
            ...
            //อาการเสีย
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[18] . $rowCell, );
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[18] . $rowCell . ':' . $columnCharacter[18] . ($rowCell));

            //วิธีแก้
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[19] . $rowCell, );
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[19] . $rowCell . ':' . $columnCharacter[19] . ($rowCell));



            //ทีมงาน
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[20] . $rowCell, $row_staff['branch_name']);
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[20] . $rowCell . ':' . $columnCharacter[20] . ($rowCell));

            //ชาสงดูแล
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[21] . $rowCell, $row_staff['fullname']);
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[21] . $rowCell . ':' . $columnCharacter[21] . ($rowCell));

            //สถานะ
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[22] . $rowCell, );
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[22] . $rowCell . ':' . $columnCharacter[22] . ($rowCell));


            //สถานะ
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[23] . $rowCell, );
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[23] . $rowCell . ':' . $columnCharacter[23] . ($rowCell));

            //สถานะ
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[24] . $rowCell, );
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[24] . $rowCell . ':' . $columnCharacter[24] . ($rowCell));
            //สถานะ
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[25] . $rowCell, );
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[25] . $rowCell . ':' . $columnCharacter[25] . ($rowCell));

            //สถานะ
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[26] . $rowCell, );
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[26] . $rowCell . ':' . $columnCharacter[26] . ($rowCell));


            //สถานะ
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($columnCharacter[27] . $rowCell, );
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($columnCharacter[27] . $rowCell . ':' . $columnCharacter[27] . ($rowCell));
...
//ตั้งชื่อไฟล์
$file_name = "ReportOverall(DEMO)" . "_" . date("d-m-Y H:i:s");
//
