...
<div class="row mb-3">
    ...
    <div class="mb-3 col-12">
        <select class="form-control select2 mb-3" style="width: 23%;" name="mechanic" id="mechanic"
            onchange="toggleBranchCare(this)">
            <option value="0">ช่าง Peaberry</option>
            <option value="1">ช่าง Dealer</option>
        </select>
    </div>

    <input type="hidden" readonly id="branch_care_id" value="" name="branch_care_id" class="form-control">
    <div class="col-mb-3 col-3" id="branch_care_container">
        <label>ทีมดูแล</label>
        <font color="red">**</font>
        <div class="input-group">
    ...
    <!-- Peaberry -->
    <div class="mb-3 col-3 user_list" id="user_list" style="">
    ...
    <!-- Dealer -->
    <div class="mb-3 col-3 user_list2" id="user_list2" style="display:none;">
        <label>ช่างผู้รับผิดชอบ </label>
        <font color="red">**</font>
        <select class="form-control select2 mb-3" style="width: 100%;" name="dealer_mechanic" id="dealer_mechanic">
            <option value="">กรุณาเลือกช่าง</option>
            <?php
            $sql = "SELECT * FROM tbl_user WHERE dealer_status = '1'";
            $res = mysqli_query($connect_db, $sql);
            if (mysqli_num_rows($res) > 0) {
                while ($row = mysqli_fetch_assoc($res)) {
                    if ($row['dealer_status'] == 1) {
                        ?>
    ...
    <div class="mb-3 col-3">
        ...
        <div class="input-group date">
            ...
            <input type="text" id="appointment_date" readonly name="appointment_date" class="form-control datepicker"
...
<div class="row mb-3">
    ...
    <div class="mb-3 col-3">
        <label>วันที่เริ่มประกัน</label>
        <input type="text" readonly id="o_warranty_start_date" value="" name="o_warranty_start_date"
            class="form-control">
    </div>

    <div class="mb-3 col-3">
        <label>วันที่หมดประกัน</label>
        <input type="text" readonly id="o_warranty_expire_date" value="" name="o_warranty_expire_date"
            class="form-control">
    </div>
</div>
...
<script>
    function toggleBranchCare() {

        var mechanicValue = document.getElementById('mechanic').value;
        var branchCareContainer = document.getElementById('branch_care_container');

        if (mechanicValue == '0') { // ถ้าเลือกช่าง Peaberry
            branchCareContainer.style.display = 'block'; // แสดงฟิลด์ "ทีมดูแล"
        } else {
            branchCareContainer.style.display = 'none'; // ซ่อนฟิลด์ "ทีมดูแล" เมื่อเลือกช่าง Dealer
        }
        ...
        if (mechanicValue == '1') { // ถ้าเลือกช่าง Dealer
            userlist.style.display = 'none';
            userlist2.style.display = 'block';
        } else {
            userlist.style.display = 'block';
            userlist2.style.display = 'none';
