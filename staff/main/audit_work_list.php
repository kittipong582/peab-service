...
<div class="p-1">
    ...
    <div class="row">
        ...
        <div class="col-5 mb-2">
            <input type="text" class="form-control datepicker text-center" name="start_date" id="start_date"
                value="<?php echo date("d/m/Y", strtotime("now -7 days")); ?>" readonly onchange="Getdata();">
        </div>
        <div class="col-2 mb-2">
            <center><label style="padding-top: 1ex;"><b> ถึง </b></label></center>
        </div>
        <div class="col-5 mb-2">
            <input type="text" class="form-control datepicker text-center" name="end_date" id="end_date"
                value="<?php echo date("d/m/Y", strtotime("now +7 days")); ?>" readonly onchange="Getdata();">
        </div>

        <div class="col-12 mb-2">
            <input type="text" class="form-control mb-2" placeholder="ค้นหาเลขที่ร้าน เช่น (DD0686,CD0112)" id="search"
                name="search" value="">
...
<script>
    ...
    function Getdata() {
        ...
        $.ajax({
            type: 'POST',
            url: "ajax/audit_work_list/Getdata.php",
            data: {
    ...
    function GetModalStart(group_id) {
        ...
        $("#myModal").modal("show");
        $("#showModal").load("ajax/audit_work_list/modal_start.php", {
            group_id: group_id,
        });
    }

    function Start_Audit(job_id, audit_id, group_id) {
        ...
        swal({
            ...
            $.ajax({
                type: "POST",
                url: "ajax/audit_work_list/Update_Start.php",
                data: {
                    job_id: job_id,
                    audit_id: audit_id,
                    group_id: group_id
                ...
                success: function (data) {
                    if (data.result == 1) {
                        swal({
                            ...
                            swal.close();
                            window.location.href = 'audit_form.php?user=' + user_id + '&job=' + job_id;;
                        });
    ...
    function ModalClose(group_id) {
        $("#myModal").modal("show");
        $("#showModal").load("ajax/audit_work_list/close_audit.php", {
            group_id: group_id
        });
    }
    ...
    function SubmitColseJob(group_id) {
        ...
        if (sig_validate == 0) {
            ...
            swal({
                ...
                $.ajax({
                    type: "POST",
                    url: "ajax/audit_work_list/update_close_job.php",
                    data: {
                        group_id: group_id,
                        signature: img_data
                    },
