...
WHERE a.responsible_user_id = '$user_id'  AND close_datetime IS NULL AND a.cancel_datetime IS NULL AND a.appointment_date <= '$date_now'";
...
LEFT JOIN tbl_customer_branch b ON a.customer_branch_id = b.customer_branch_id 
WHERE a.responsible_user_id = '$user_id' AND close_datetime IS NULL AND a.appointment_date <= '$date_now')";
...
LEFT JOIN tbl_product_model d ON c.model_id = d.model_id
LEFT JOIN tbl_sub_job_type k ON k.sub_job_type_id = a.sub_job_type_id
WHERE job_id IN (SELECT job_id FROM tbl_job_oh WHERE user_id = '$user_id' AND close_datetime IS NULL AND a.appointment_date <= '$date_now')
AND a.job_type = 4 ";
