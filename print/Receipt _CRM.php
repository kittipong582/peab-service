...
LEFT JOIN tbl_branch e ON e.branch_id = d.branch_id	
 WHERE a.job_id = '$job_id'";
    ...
    while ($row_group2 = mysqli_fetch_assoc($rs_group2)) {
        ...
        $sql = "SELECT * FROM (SELECT a.spare_part_id as num1,a.job_id as num2,b.spare_part_code as num3,b.spare_part_name as num4,a.quantity as num5,a.unit_price as num6,1 as num7,a.insurance_type as num8 FROM tbl_job_spare_used a
            LEFT JOIN tbl_spare_part b ON a.spare_part_id = b.spare_part_id
            UNION
            SELECT a.job_income_id as num1,a.job_id as num2,b.income_code as num3,b.income_type_name as num4,a.quantity as num5,a.income_amount as num6,2 as num7,'not_spare' as num8 FROM tbl_job_income a
        ...
        while ($row = mysqli_fetch_assoc($rs)) {
            ...
            $sql_wrt = "SELECT * FROM tbl_warranty_type WHERE warranty_type_id = '{$row['num8']}'";
            $rs_wrt = mysqli_query($connect_db, $sql_wrt) or die($connect_db->error);
            $row_wrt = mysqli_fetch_assoc($rs_wrt);
    
    
            if (mysqli_num_rows($rs_wrt) != 0) {
                $insurance_type = "(" . $row_wrt['warranty_type_name'] . ")";
            }
            ...
            if (array_keys(array_column($detail_array, 'id'), $row['num1']) == true) {
                ...
                $new_total = $detail_array[$temp_key]['total'] + $total;
...
SELECT a.job_income_id as num1,a.job_id as num2,b.income_code as num3,b.income_type_name as num4,a.quantity as num5,a.income_amount as num6,2 as num7, '99' as num8 FROM tbl_job_income a
LEFT JOIN tbl_income_type b ON b.income_type_id = a.income_type_id
    ...
    while ($row = mysqli_fetch_assoc($rs)) {
        ...
        $insurance_type = "";
        // if ($row['num8'] == 1) {
        //     $insurance_type = '(ประกัน 105 วัน)';
        // } else if ($row['num8'] == 2) {
        //     $insurance_type = '(ประกัน 1 ปี)';
        // } else if ($row['num8'] == 3) {
        //     $insurance_type = '(ประกัน 2 ปี)';
        // } else if ($row['num8'] == 4) {
        //     $insurance_type = '(ฟรีในรอบ PM)';
        // }
       ...
       $sql_wrt = "SELECT * FROM tbl_warranty_type WHERE warranty_type_id = '{$row['num8']}'";
        ...
        $pdf->SetXY(0.14, 0); //รายการ
        $pdf->MultiCell(1.2, $c, iconv('UTF-8', 'cp874', $row['num4']), 0, 'L', 0);


        $pdf->SetXY(1.2, 0); //รายการ
        $pdf->MultiCell(1.5, $c, iconv('UTF-8', 'cp874', $insurance_type), 0, 'L', 0);
    ...
    foreach ($detail_array as $row) {
        ...
        $pdf->SetXY(0.14, 0); //รายการ
        $pdf->MultiCell(1.2, $c, iconv('UTF-8', 'cp874', $row['list_detail']), 0, 'L', 0);

        $pdf->SetXY(1.18, 0); //รายการ
        $pdf->MultiCell(1.5, $c, iconv('UTF-8', 'cp874', $row['insurance_type']), 0, 'L', 0);
...
$pdf->SetXY(0, 0); //Reference No.   
$pdf->MultiCell(2.65, $b + 4.72, iconv('UTF-8', 'cp874', "หากต้องการแก้ใบกำกับภาษีรบกวนแจ้งกลับภายใน 7 วัน"), 0, 'R', 0);

$pdf->SetXY(0, 0); //Reference No.   
$pdf->MultiCell(2.43, $b + 5.05, iconv('UTF-8', 'cp874', "มิฉะนั้นจะถือว่าเอกสารนี้ถูกต้องสมบูรณ์แล้ว"), 0, 'R', 0);
